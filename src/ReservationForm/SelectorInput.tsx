...
export const SelectorInput = ({ label, placeholder, items, onChange, value, disabled, error }: SelectorInputProps) => {
    ...
    <>
      ...
      <div className="relative">
        <Input
          ...
          trailing={<ArrowDownIcon className="text-black" width={25} opacity={0.3} />}
            ...
            {items.map((item) => (
              <li
                className={`cursor-pointer px-6 py-4 text-sm ${
                  item.value === value ? "text-gray-500 bg-gray-300" : "hover:bg-primary hover:text-white font-light"
                }`}
                key={item.value}
                onClick={
                  onChange &&
